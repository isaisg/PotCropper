...
distanceaxis<-function(point1,point2)
{
  resultx<-abs(point2[1]-point1[1])
  resulty<-abs(point2[2]-point1[2])
  resultado<-c(resultx,resulty)
  return(resultado)
}
combn_distances_axis<-function(list_vars){
  ...
  for(i in 1:ncol(combi_individuals)){
    ...
    p1<-as.vector(sapply(part_1,"[[","values"))
    p2<-as.vector(sapply(part_2,"[[","values"))
    res<-distanceaxis(p1,p2)
    distan_comb[[length(distan_comb)+1]]<-list(name=paste(combi_individuals[,i],collapse="_"),values=res)
  }
  return(distan_comb)
...
determine_h1_h2_h3<-function(dir,file){
  ...
  axes<-combn_distances_axis(indiv_vars)
  ...
  h3<-machear[machear!=h1]
  h2<-indiv_variables[indiv_variables!=h1 & indiv_variables !=h3]
...
subset_pots<-function(file_image=NA,dir="./",outdir="./",file_txt=paste(unlist(strsplit(paste(file_image),"[.]"))[1],"txt",sep="."),barcode_position="outside",pots_positions=c(1:16),size=325,suffix=barcode_position){
  if (is.na(file_image)){
    ...
    for (i in 1:length(imfiles)){
      ...
      for(i in pots_positions){
        ...
        writeImage(section,paste(paste(outdir,paste(file_id,"pot",suffix,i,sep="_"),sep=""),"jpeg",sep="."))
      }
    }
  }
